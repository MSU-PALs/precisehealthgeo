...
gdf = gpd.read_file(
    os.path.join(neighborhoods, "PRECISE_Village_Boundaries.shp"),
).rename(columns={"Village co": "village_code", "Village": "village_name"})
...
for country in ["gambia"]:
    ...
    # function to extract daily temperature for each neighborhood
    def extract_temperature(image):
        exposure_date = ee.Date(image.get("system:time_start")).format("YYYY-MM-dd")
        # compute zonal mean for each neighborhood
    ...
    # extract for study period for each neighborhood
    dataset = images.map(extract_temperature).flatten()

    # # convert to pandas dataframe
    dataset = geemap.ee_to_df(dataset).rename(columns={"mean": "ERA-5_mean"})

    dataset.to_pickle(os.path.join(outdir, "ERA-5_daily_village_mean.pkl"))
