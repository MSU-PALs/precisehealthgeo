...
data_dir <- paste0(root_dir, "/data/processed/heat_days/")

extreme_heat_data <- function(level) {
    out_df <- data.frame()
    ...
    df_long <- out_df %>%
        pivot_longer(
            cols = c("hd_p75", "hd_p90", "hd_p95"),
            names_to = "heat_def",
            values_to = "proportion"
        ) %>%
        mutate(heat_def = toupper(heat_def)) %>%
        mutate(proportion = 100*proportion) # convert to percentage
  
    return(df_long)
}
# Define x-axis limits for each heat definition
xlims <- tibble::tibble(
    heat_def = c("HD_P75", "HD_P90", "HD_P95"),
    xmin = c(0, 0, 0),
    xmax = c(100, 60, 25)
)
# Filter data based on x-axis limits for each heat definition
plot_data <- extreme_heat_data(level = "village") %>%
...
# Save the plot
ggsave(
    filename = paste0(root_dir, "/reports/ridgeline_heatdays-village.png"),
    plot = final_plot,
    width = 12,
    height = 9,
    dpi = 600,
