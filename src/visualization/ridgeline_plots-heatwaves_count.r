...
heatwave_data <- function(level) {
    ...
    df_long <- out_df %>%
        rename_with(~ sub("_num$", "", .x), ends_with("_num")) %>%
        pivot_longer(
...
xlims <- tibble::tibble(
    ...
    xmax = c(20, 20, 15, 30, 20, 10, 15, 10, 10)
)
...
# Save the plot
ggsave(
    filename = paste0(root_dir, "/reports/ridgeline_heatwave-counts-village.png"),
    plot = final_plot,
    width = 12,
    height = 9,
    dpi = 600,
